...
//    ModuleResolution moduleResolution = ModuleResolution.empty();
    ...
    static {
        try {
            rsrc = ResourceBundle.getBundle("org.cq.jartool.resource.jar");
...
//                    Files.move(path, Paths.get(fname), StandardCopyOption.REPLACE_EXISTING);
                    Files.move(path, Paths.get(foutname), StandardCopyOption.REPLACE_EXISTING);
                } else {
                   ...
                   JarIndex jarIndex) throws IOException
    {
        ...
        while ((e = zis.getNextEntry()) != null) {
            ...
            boolean isModuleInfoEntry = false;
    ...
    private void addIndex(JarIndex index, ZipOutputStream zos)
            throws IOException
    {
        ZipEntry e = new ZipEntry(JarIndex.INDEX_NAME);
        e.setTime(System.currentTimeMillis());
        if (flag0) {
            ...
            throws IOException
    {
        ...
        String releases = Arrays.stream(infos)
                ...
                .map(JarTool::versionFromEntryName)
                .collect(joining(" "));
        if (!releases.equals(""))
