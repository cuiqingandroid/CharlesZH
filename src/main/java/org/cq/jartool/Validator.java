...
import static org.cq.jartool.JarTool.*;

final class Validator {

    private final Map<String,FingerPrint> classes = new HashMap<>();
    private final JarTool main;
    ...
    private Validator(JarTool jarTool, ZipFile zf) {
        this.main = jarTool;
        this.zf = zf;
    }

    static boolean validate(JarTool jarTool, ZipFile zf) throws IOException {
        return new Validator(jarTool, zf).validate();
    }
        ...
        if (ze != null) {
            try (InputStream jis = zf.getInputStream(ze)) {
                ...
                if (base == null) {
                    ...
                    md.exports().stream().map(ModuleDescriptor.Exports::source).forEach(concealedPkgs::remove);
                    md.opens().stream().map(ModuleDescriptor.Opens::source).forEach(concealedPkgs::remove);
                    // must have the implementation class of the services it 'provides'.
                    if (md.provides().stream().map(ModuleDescriptor.Provides::providers)
                          .flatMap(List::stream)
                ...
                if (!base.requires().equals(md.requires())) {
                    Set<ModuleDescriptor.Requires> baseRequires = base.requires();
                    ...
                    for (ModuleDescriptor.Requires r : baseRequires) {
                        Set<ModuleDescriptor.Requires> mdRequires = md.requires();
